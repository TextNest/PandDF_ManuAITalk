...
# [사용 예시]
#   - 전체 문서에 대해 텍스트 청킹:
#       (.venv) > python -m src.text_chunker
#
#   - 특정 문서만 처리 (예: SAH001):
#       (.venv) > python -m src.text_chunker --doc-id SAH001
#
#   - 기존 청크 JSONL을 무시하고 다시 생성:
#       (.venv) > python -m src.text_chunker --force
#
# ============================================================
...
# 이 파일(src/text_chunker.py)을 기준으로 프로젝트 루트 계산
...
def iter_paragraphs_from_pages(pages: List[Dict[str, Any]]) -> Iterable[Paragraph]:
    ...
    for page in pages:
        ...
        def flush_buffer() -> None:
            ...
            if text:
                yield_para = Paragraph(page=page_no, text=text)
                # 내부 함수에서 yield 를 쓸 수 없으므로
                paragraphs.append(yield_para)
            buffer.clear()
...
def process_one_normalized_file(
    ...
    Args:
        md_path: 입력 normalized 마크다운 파일 경로
        force: 기존 청크 JSONL이 있어도 덮어쓸지 여부
        target_chars: 청크 목표 길이(문자 수)
        max_chars: 청크 최대 길이(문자 수)
    """
    doc_id = md_path.stem
