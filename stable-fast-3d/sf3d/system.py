        ...
        with torch.no_grad():
            with torch.autocast(
                ...
                for i, mesh in enumerate(meshes):
                    ...
                    # Build textures using a manual rasterizer with torch
                    uvs = mesh.v_tex
                    ...
                    ii, jj = torch.meshgrid(
                        torch.arange(resolution, device=uvs.device),
                        torch.arange(resolution, device=uvs.device),
                        indexing="xy",
                    )
                    grid_points = torch.stack([ii, jj], dim=-1).float() / (resolution - 1)
                    grid_points = grid_points.view(-1, 2)

                    # Get UV coordinates for each face
                    face_uvs = uvs[faces]  # (F, 3, 2)
                    
                    # Calculate barycentric coordinates for all grid points against all faces
                    # This is computationally intensive.
                    # Using a simplified check: point in triangle bbox first
                    min_uv, _ = face_uvs.min(dim=1)
                    max_uv, _ = face_uvs.max(dim=1)

                    possible_faces = []
                    for i in range(faces.shape[0]):
                        # Check if any grid point is within the bounding box of the face's UVs
                        in_bbox = (grid_points >= min_uv[i]) & (grid_points <= max_uv[i])
                        in_bbox = in_bbox.all(dim=1)
                        if in_bbox.any():
                            possible_faces.append(i)
                    
                    possible_faces = torch.tensor(possible_faces, device=uvs.device, dtype=torch.long)
                    face_uvs_subset = face_uvs[possible_faces]

                    v0 = face_uvs_subset[:, 1] - face_uvs_subset[:, 0]
                    v1 = face_uvs_subset[:, 2] - face_uvs_subset[:, 0]
                    
                    # Expand grid_points and face_uvs for broadcasting
                    grid_points_expanded = grid_points.unsqueeze(0) # (1, N, 2)
                    face_uvs_0_expanded = face_uvs_subset[:, 0].unsqueeze(1) # (F_sub, 1, 2)
                    
                    v2 = grid_points_expanded - face_uvs_0_expanded # (F_sub, N, 2)
                    ...
                    denom[denom.abs() < 1e-8] = 1e-8

                    w1 = (d11 * d20 - d01 * d21) / denom
                    w2 = (d00 * d21 - d01 * d20) / denom
                    w0 = 1.0 - w1 - w2

                    # Find points inside a triangle
                    inside = (w0 >= 0) & (w1 >= 0) & (w2 >= 0) # (F_sub, N)

                    # Find the first face that each grid point belongs to
                    face_indices = torch.full((grid_points.shape[0],), -1, dtype=torch.long, device=uvs.device)
                    bary_coords = torch.zeros((grid_points.shape[0], 3), dtype=torch.float32, device=uvs.device)

                    # This loop is slow, but a vectorized `findfirst` is not trivial in torch
                    for p_idx in range(grid_points.shape[0]):
                        first_match = torch.where(inside[:, p_idx])[0]
                        if len(first_match) > 0:
                            face_idx = first_match[0]
                            face_indices[p_idx] = possible_faces[face_idx]
                            bary_coords[p_idx] = torch.stack([w0[face_idx, p_idx], w1[face_idx, p_idx], w2[face_idx, p_idx]], dim=0)

                    bake_mask = (face_indices != -1)
                    
                    # Interpolate attributes for pixels on the mesh
                    def interpolate_attribute(attr):
                        face_attr = attr[faces[face_indices[bake_mask]]]
                        interpolated_attr = (face_attr * bary_coords[bake_mask].unsqueeze(-1)).sum(dim=1)
                        return interpolated_attr

                    gb_pos = interpolate_attribute(mesh.v_pos)
                    ...
                    gb_nrm = F.normalize(interpolate_attribute(mesh.v_nrm), dim=-1)
                    ...
                    for k, v in mat_out.items():
                        ...
                        if v.shape[0] == 1:
                            ...
                            if k == "normal":
                                ...
                                tng = interpolate_attribute(mesh.v_tng)
                                gb_tng = tng
