# ============================================================
#  File: module/rag_pipeline/image_result_selector.py
# ============================================================
# [모듈 개요]
...
#
# [사용 예]
#   from module.rag_pipeline.image_result_selector import select_image_results
#
...
def select_image_results(
    ...
    for ch in retrieved_chunks:
        ...
        # RetrievedChunk 에는 id 필드 대신 uid 가 있으므로,
        # uid → meta["uid"] → meta["chunk_id"] → 폴백 순으로 사용
        chunk_id = (
            getattr(ch, "uid", None)
            or meta.get("uid")
            or meta.get("chunk_id")
            or f"{doc_id}:figure:{figure_index or 0}"
        )
