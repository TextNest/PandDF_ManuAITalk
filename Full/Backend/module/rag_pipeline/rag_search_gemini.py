# ============================================================
#  File: module/rag_pipeline/rag_search_gemini.py
# ============================================================
# [모듈 개요]
...
#
# [실행 예시] (Backend 루트에서)
#   (.venv) > python -m module.rag_pipeline.rag_search_gemini
#
# ============================================================
...
# 이 파일(module/rag_pipeline/rag_search_gemini.py)을 기준으로 Backend 루트 계산
#   .../Full/Backend/module/rag_pipeline/rag_search_gemini.py
...
def load_gemini_client() -> genai.Client:
    ...
    - Backend/.env 가 존재하면 먼저 로드한다.
    - GEMINI_API_KEY 또는 GOOGLE_API_KEY 를 사용.
    """
    if ENV_FILE_PATH.exists():
        load_dotenv(ENV_FILE_PATH, override=False)
        logger.info("환경 변수 로드 완료: %s", ENV_FILE_PATH)
    else:
        # fallback: 현재 프로세스 환경만 사용
        logger.warning(".env 파일이 존재하지 않습니다: %s", ENV_FILE_PATH)
...
class RagSearcher:
    ...
    def search(
        ...
        # ------------------------------------------------------------------
        # 2단계: doc_id_filter가 없거나, 매칭 실패 → 전체 코퍼스 검색
        # ------------------------------------------------------------------
